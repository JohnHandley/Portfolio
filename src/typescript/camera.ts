...
export class Camera {
    ...
    setAngle() {
        ...
        this.angle.set = (name: string) => {
            const angle = this.angle.items[name]
    ...
    setZoom() {
        ...
        document.addEventListener('mousewheel', (event: any) => {
            this.zoom.targetValue += event.deltaY * 0.001
        ...
        this.renderer.domElement.addEventListener('touchstart', (event: any) => {
            if (event.touches.length === 2) {
                this.zoom.touch.startDistance = Math.hypot(event.touches[0].clientX - event.touches[1].clientX, event.touches[0].clientX - event.touches[1].clientX)
        ...
        this.renderer.domElement.addEventListener('touchmove', (event: any) => {
            if (event.touches.length === 2) {
                ...
                const distance = Math.hypot(event.touches[0].clientX - event.touches[1].clientX, event.touches[0].clientX - event.touches[1].clientX)
    ...
    setPan() {
        ...
        this.pan.down = (x: number, y: number) => {
            if (!this.pan.enabled) {
                return
            }
        ...
        this.pan.move = (x: number, y: number) => {
            if (!this.pan.enabled) {
                return
            }
        ...
        window.addEventListener('mousedown', (event: any) => {
            this.pan.down(event.clientX, event.clientY)
        })

        window.addEventListener('mousemove', (event: any) => {
            this.pan.move(event.clientX, event.clientY)
        })
        ...
        this.renderer.domElement.addEventListener('touchstart', (event: any) => {
            if (event.touches.length === 1) {
                this.pan.down(event.touches[0].clientX, event.touches[0].clientY)
            }
        })

        this.renderer.domElement.addEventListener('touchmove', (event: any) => {
            if (event.touches.length === 1) {
                this.pan.move(event.touches[0].clientX, event.touches[0].clientY)
            }
        })
